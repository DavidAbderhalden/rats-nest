...
class CustomersModel(BaseModel):
    ...
    registration_time: Mapped[int] = mapped_column(Integer, insert_default=func.current_timestamp())

    # enums
    role: Mapped[Roles] = mapped_column(Enum(Roles), insert_default=Roles.USER)

    # foreign keys
    delivery_address_id: Mapped[int] = mapped_column(Integer, ForeignKey('address.address_id'))
    home_address_id: Mapped[int] = mapped_column(Integer, ForeignKey('address.address_id'))
