...
from app.utils import SQLStrEnum, pascal_to_snake_case
...
class CustomersModel(BaseModel):
    ...
    birthdate: Mapped[date] = mapped_column(String)
    registration_date: Mapped[int] = mapped_column(Integer, insert_default=func.current_timestamp())

    # enums
    role: Mapped[Roles] = mapped_column(SQLStrEnum(Roles), insert_default=Roles.USER)
...
class BillingProfilesModel(BaseModel):
    billing_profile_id: Mapped[id] = mapped_column(Integer, primary_key=True, autoincrement=True, index=True)
    card_number: Mapped[int] = mapped_column(Integer)
    card_name: Mapped[str] = mapped_column(String(20))
    expiry_date: Mapped[date] = mapped_column(String)
