...
from app.services.libs import AddressService
...
async def search_address(addr: str):
    service_success: ServiceOperationsSuccess[Type[list[str]]] = \
        await base_controller.read(body=addr, response_model=list[str])
...
async def create_address(body: AddressGlueCreate):
    service_success: ServiceOperationsSuccess[Type[AddressGlueRead]] = \
        await base_controller.create(body=body, response_model=AddressGlueRead)
    return service_success.data
